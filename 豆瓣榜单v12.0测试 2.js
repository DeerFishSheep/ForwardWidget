...
// @name         豆瓣榜单大全
// @version      12.0.0
// @description  顶部引入用户自定义配置区，支持一键更改模块排序与快捷片单增删
// @author       YourName
// =============UserScript=============

// ==========================================
// ⚙️ 用户自定义配置区 (在此处修改即可全局生效)
// ==========================================
const USER_CONFIG = {
    // 1. 模块排序配置 (数字越小，排序越靠前)
    MODULE_SORT_ORDER: {
    ...
    // 2. 自定义片单快捷列表 (格式为 "片单名称": "片单链接")
    // 你可以随意增加或删除下方的行
    CUSTOM_PLAYLISTS: {
        "IMDB科幻电影Top 200": "https://www.douban.com/doulist/240612/?dt_dapp=1",
        // 示例：如需增加，直接在下方追加即可
        "漫威电影宇宙排序": "https://www.douban.com/doulist/3946029/"
    }
};
...
// 将你的键值对配置转换为底层播放器需要的 placeholders 数组格式
const generatedPlaceholders = Object.keys(USER_CONFIG.CUSTOM_PLAYLISTS).map(title => ({
    title: title,
    value: USER_CONFIG.CUSTOM_PLAYLISTS[title]
}));
        ...
        params: [
            {
                ...
                placeholders: generatedPlaceholders // 💡 动态引入配置区的片单
            },
...
const sortedModules = rawModules.sort((a, b) => {
    const orderA = USER_CONFIG.MODULE_SORT_ORDER[a.title] ?? 999;
    const orderB = USER_CONFIG.MODULE_SORT_ORDER[b.title] ?? 999;
    return orderA - orderB;
});

// 重构覆盖 modules
WidgetMetadata.modules = sortedModules;
